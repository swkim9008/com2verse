...
using Com2Verse.Project.Runtime.Scripts.Avatar.Etc;
using Com2Verse.UI;
...
#pragma warning restore 0649
		...
		public static FileBrowser Instance
		{
			get
			{
				...
				// 	if (!m_instance)
				// 		m_instance = new GameObject("SimpleFileBrowser").AddComponent<FileBrowser>();
				//
...
#pragma warning restore 0414
		...
		private bool ActiveSelf => !_view.IsReferenceNull() && _view.VisibleState == GUIView.eVisibleState.OPENED || _view.VisibleState == GUIView.eVisibleState.OPENING;
		#endregion
		...
		private void LateUpdate()
		{
			...
			{
				if (_view.IsReferenceNull() || _view.VisibleState != GUIView.eVisibleState.OPENED) return;
		...
		private static void ShowView(Action<GUIView> onShowView)
		{
			if (_view.IsReferenceNull())
			{
				UIManager.Instance.CreatePopup("SimpleFileBrowserCanvas", view =>
				{
					if (view.IsUnityNull()) return;

					_view = view;
					_view.Show();
					onShowView?.Invoke(_view);
				}).Forget();
			}
		}
		public void Hide()
		...
		{
			if (_view.IsReferenceNull()) return;
		...
		public static bool ShowSaveDialog( OnSuccess onSuccess, OnCancel onCancel,
										   ...
										   Action onBeforeShow = null, Action onAfterShow = null)
		{
			return ShowDialogInternal( onSuccess, onCancel, pickMode, allowMultiSelection, pickMode != PickMode.Folders, initialPath, initialFilename, title, saveButtonText, onBeforeShow, onAfterShow);
		}
		...
		public static bool ShowLoadDialog( OnSuccess onSuccess, OnCancel onCancel,
										   ...
										   Action onBeforeShow = null, Action onAfterShow = null)
		{
			return ShowDialogInternal( onSuccess, onCancel, pickMode, allowMultiSelection, false, initialPath, initialFilename, title, loadButtonText, onBeforeShow, onAfterShow);
		}
		...
		private static bool ShowDialogInternal( OnSuccess onSuccess, OnCancel onCancel,
												...
												Action onBeforeShow, Action onAfterShow)
		{
			...
			ShowView(view =>
			{
				m_instance = view.GetComponent<FileBrowser>();
				if (m_instance.IsReferenceNull())
				{
					C2VDebug.LogWarning($"FileBrowser component missing");
					view.Hide();
					return;
				}
