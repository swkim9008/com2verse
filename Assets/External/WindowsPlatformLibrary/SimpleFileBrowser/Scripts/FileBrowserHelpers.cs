...
namespace SimpleFileBrowser
{
	...
	public static class FileBrowserHelpers
	{
		...
		public static FileSystemEntry[] GetEntriesInDirectory( string path, bool extractOnlyLastSuffixFromExtensions )
		{
			...
			try
			{
				...
				for( int i = 0; i < items.Length; i++ )
				{
					try
					{
						result[index] = new FileSystemEntry( items[i], GetExtensionFromFilename( items[i].Name, extractOnlyLastSuffixFromExtensions ) );
						index++;
					}
		...
		private static string GetExtensionFromFilename(string filename, bool extractOnlyLastSuffix)
		{
