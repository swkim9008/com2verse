...
namespace Com2verseEditor.UnityAssetTool
{
    ...
    {
        private Object currentTMPFont;
        private Object replaceTMPFont;

        //private int targetFontSize = 0;
        //-----------------------------------------------------------------------------------------------------------
        private UnityEngine.TextAsset TMPfontDataText = null;
        private UnityEngine.Texture2D TMPfontTexture = null;
        private UnityEngine.Font customTMPFont = null;
        //-----------------------------------------------------------------------------------------------------------
  

        void UIGroup_TextMeshPro()
        {
            ...
            EditorGUILayout.LabelField("From Font", EditorStyles.boldLabel);
            this.currentTMPFont = EditorGUILayout.ObjectField(this.currentTMPFont, typeof(TMP_FontAsset), true);

            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button("설정", GUILayout.Height(30))) { this.currentTMPFont = GetCurrentTMPTextFont(); }
            EditorGUI.BeginDisabledGroup(null == this.currentTMPFont);
            if (GUILayout.Button("동일 폰트 Text 선택", GUILayout.Height(30))) { SelectTMPTextByFont((TMP_FontAsset)this.currentTMPFont); }
            ...
            EditorGUILayout.BeginVertical();
            EditorGUILayout.LabelField("To Font", EditorStyles.boldLabel);
            //EditorGUI.BeginDisabledGroup(null == this.currentTMPFont);
            this.replaceTMPFont = EditorGUILayout.ObjectField(this.replaceTMPFont, typeof(TMP_FontAsset), true);
            //EditorGUI.EndDisabledGroup();
            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button("설정", GUILayout.Height(30))) { this.replaceTMPFont = GetCurrentTMPTextFont(); }
            EditorGUI.BeginDisabledGroup(null == this.replaceTMPFont);
            if (GUILayout.Button("동일 폰트 Text 선택", GUILayout.Height(30))) { SelectTMPTextByFont((TMP_FontAsset)this.replaceTMPFont); }
            EditorGUI.EndDisabledGroup();
            EditorGUILayout.EndHorizontal();
            ...
            EditorGUI.BeginDisabledGroup(null == this.currentTMPFont || null == this.replaceTMPFont);
            GUI.color = Color.cyan;
            if (GUILayout.Button("From Font를 To Font로 교체", GUILayout.Height(40)))
            {
                GameObject target = Selection.activeObject as GameObject;
                if (target == null) return;
                
                TextMeshProUGUI[] targetTextList = target.GetComponentsInChildren<TextMeshProUGUI>(true);

                foreach (TextMeshProUGUI text in targetTextList)
                {
                    if (text.font == (TMP_FontAsset)this.currentTMPFont)
                    {
                        text.font = (TMP_FontAsset)this.replaceTMPFont; 
                    };
                }
            }
            ...
            if (GUILayout.Button("동일 Size Text 선택", GUILayout.Height(30))) { SelectTMPTextByFontSize(this.targetFontSize); }
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.Space(10);
            GUI.color = Color.yellow;
            if (GUILayout.Button("Missing 폰트 Text 선택", GUILayout.Height(30))) { SelectTMPTextByFont(null); }
            GUI.color= Color.white;
            //if (GUILayout.Button("UIText RayCast Target OFF", GUILayout.Height(30))) { UITMPTextRaycastTargetOff<TMP_Text>(); }
        }


        public void UIGroup_CreateTMPCustomFont()
        {
            EditorGUILayout.BeginHorizontal();

            EditorGUILayout.BeginVertical(GUILayout.Width(300));
            this.customTMPFont = (Font)EditorGUILayout.ObjectField("Custom font", this.customTMPFont, typeof(Font), true);
            this.TMPfontDataText = (TextAsset)EditorGUILayout.ObjectField("Font Data", this.TMPfontDataText, typeof(TextAsset), true );
            this.TMPfontTexture = (Texture2D)EditorGUILayout.ObjectField("Font Texture", this.TMPfontTexture, typeof(Texture2D), true);
            EditorGUILayout.EndVertical();

            if (GUILayout.Button("Create Custom Font", GUILayout.Height(30))) { CreateTMPCustomFont(); }
        ...
        // Label ---------------------------------------------------------------------------------------
        // ---------------------------------------------------------------------------------------------
        private TMP_FontAsset GetCurrentTMPTextFont()
        {
            GameObject target = Selection.activeObject as GameObject;
        ...
        void SelectTMPTextByFont(TMP_FontAsset _font)
        {
            ...
            TextMeshProUGUI[] targetTextList = targetObj.GetComponentsInChildren<TextMeshProUGUI>(true);

            foreach (TextMeshProUGUI _text in targetTextList)
            {
                if ((TMP_FontAsset)_text.font == _font)
                {
        ...
        void SelectTMPTextByFontSize(int _fontSize)
        {
            GameObject targetObj = Selection.activeObject as GameObject;
            if (null == targetObj) return;
        ...
        private void UITMPTextRaycastTargetOff<T>() where T : MaskableGraphic
        {
            GameObject rootObj = Selection.activeObject as GameObject;
            if (null == rootObj) return;

            TextMeshProUGUI[] textList = rootObj.GetComponentsInChildren<TextMeshProUGUI>();
        ...
        private void CreateTMPCustomFont()
        {
            if (null == this.fontDataText) return;
            if (null == this.fontTexture) return;

            CharacterInfo[] characterInfos = CreateTMPCharacterInfo(this.fontDataText);
            ...
            if (null == this.customFont)
            {
                ...
                string outFontMaterialName = string.Format("{0}.mat", Path.Combine(outPath, outName));
                ...
                string outFontSettingName = string.Format("{0}.fontsettings", Path.Combine(outPath, outName));
        ...
        private CharacterInfo[] CreateTMPCharacterInfo(TextAsset _fontData)
        {
